...
function unifiedDeal(product) {
  return {
    ...
    packageSize: product.packageSize ?? null, currentPrice: product.finalPurchasePrice ?? product.currentPrice ?? null, purchasePrice: product.finalPurchasePrice ?? product.purchasePrice ?? product.currentPrice ?? null, originalPrice: product.originalPrice ?? null, couponDiscount: product.couponDiscount ?? null, finalPurchasePrice: product.finalPurchasePrice ?? product.currentPrice ?? null,
...
function productTileExtractorScript({ dealSourceName, relevantSources, excludedSource, varietySource, priceParsingSource, filterAllowed = false }) {
  ...
  return links.map((link) => {
    ...
    const instantSavingsPrice = calculateCostcoInstantSavingsPrice(text, priceList[0] ?? null);
    const finalPurchasePrice = instantSavingsPrice.finalPurchasePrice ?? priceList[0] ?? null;
    return { supplier: 'Costco Business Center', dealSource: dealSourceName, category, productName, brand: null, sku, upc: null, packageSize: clean(text.match(/\b\d+(?:\.\d+)?\s*(?:oz|ounce|ounces|fl oz|ct|count|pack|pk|lb|lbs|gallon|gal|qt)\b(?:\s*[xX]\s*\d+)?/i)?.[0]), currentPrice: finalPurchasePrice, purchasePrice: finalPurchasePrice, finalPurchasePrice, originalPrice: instantSavingsPrice.originalPrice ?? priceList[1] ?? null, couponDiscount: instantSavingsPrice.couponDiscount ?? null, discount: extractCostcoDiscount(text), coupon: extractCostcoCoupon(text), availability: clean(text.match(/(?:in stock|out of stock|available|delivery)[^.]{0,80}/i)?.[0]), quantityLimit: clean(text.match(/(?:limit|maximum|max)\s*(?:of)?\s*\d+[^.]{0,80}/i)?.[0]), productUrl, imageUrl: absUrl(image?.currentSrc || image?.getAttribute('src')), scanDate: new Date().toISOString() };
  }).filter(Boolean);
}
